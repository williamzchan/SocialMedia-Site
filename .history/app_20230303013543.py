...
def searchComments():
	...
	cursor = conn.cursor()
	cursor.execute("SELECT U.first_name, COUNT(*) AS ccount FROM users U, commented CT, comments C WHERE C.comment_id = CT.comment_id AND U.user_id = CT.user_id AND C.comment_text = '{0}' GROUP BY U.first_name ORDER BY ccount desc".format(com))
