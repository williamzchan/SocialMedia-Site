...
def comments():
	...
	cursor.execute("SELECT album_id FROM ahasp WHERE picture_id = '{0}'".format(pid))
...
def protected():
	cursor = conn.cursor()
	cursor.execute("SELECT first_name FROM Users WHERE user_ID = '{0}'".format(getUserIdFromEmail(flask_login.current_user.id)))
	name = cursor.fetchone()[0]
	return render_template('hello.html', message = name, photos=getUsersPhotos(getUserIdFromEmail(flask_login.current_user.id)), base64=base64)
