...
def comments():
	...
	except: render_template('hello.html', message='Welecome to Photoshare')
	...
	print(comment)
	cursor = conn.cursor()
	cursor.execute("INSTER INTO comments (comment_text, commenter, date_of_comment) VALUES ('{0}', '{1}', '{2}')".format(comment, user, datetime.now().strftime('%Y-%m-%d')))
