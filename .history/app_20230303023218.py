...
def comments():
	...
	if(comment == ""):
		return render_template('hello.html', message= 'cannot comment blank')
...
def newFriends():
	cursor = conn.cursor()
	cursor.execute("SELECT FR.user2_id, COUNT(*) AS fcount FROM friends F, friends FR WHERE F.user2_id = FR.user1_id AND F.user1_id = '{0}' AND F.user2_id <> '{0}' GROUP BY FR.user2_id ORDER BY fcount desc".format(getUserIdFromEmail(flask_login.current_user.id)))
	users = cursor.fetchall()
